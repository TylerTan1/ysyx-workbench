...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc; s->dnpc = s->pc + imm; 
	...
	INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(ECALL, s->pc); IFDEF(CONFIG_ETRACE, ));
	INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd) = CSR(imm); CSR(imm) |= src1; IFDEF(CONFIG_ETRACE, ));
	INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw	 , I, R(rd) = CSR(imm); CSR(imm)  = src1; IFDEF(CONFIG_ETRACE, ));
	INSTPAT("0011000 00010 00000 000 00000 11100 11", mret	 , R, s->dnpc = isa_ret_intr(); IFDEF(CONFIG_ETRACE, ));
