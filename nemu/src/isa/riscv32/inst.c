...
void print_csr(word_t imm, word_t value) {
	switch (imm) {
		case 0x300: printf("write 0x%08x to mstatus\n", value); break;
		case 0x305: printf("write 0x%08x to mtvec\n"  , value);   break;
		case 0x341: printf("write 0x%08x to mepc\n"   , value);    break;
		case 0x342: printf("write 0x%08x to mcause\n" , value);  break;
		default:
			printf("csr address = %u\n, which does not exist\n", imm);
			assert(0);
	}
...
void print_mr(word_t addr, int len, word_t data) {
#ifdef CONFIG_MTRACE_READ
	switch (len) {
		case 1: printf("Read 0x%02x from 0x%08x\n", data, addr); break;
		case 2: printf("Read 0x%04x from 0x%08x\n", data, addr); break;
		case 4: printf("Read 0x%08x from 0x%08x\n", data, addr); break;
		default: 
			printf("read %d bytes, which is impossible\n", len);
			assert(0);
	}
#endif
}
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(rd) = s->pc + imm);
	...
	INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (src1 + imm) & (~1); 
			IFDEF(CONFIG_FTRACE, if (s->isa.inst == 0x00008067) print_ret(s->pc); else if (rd == 1) print_call(s->pc, s->dnpc) )); 
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1); print_mr(src1 + imm, 1, R(rd)));
	INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb		 , I, R(rd) = SEXT(Mr(src1 + imm, 1), 8); print_mr(src1 + imm, 1, R(rd)));
	INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + imm, 2), 16); print_mr(src1 + imm, 2, R(rd)));
	INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 + imm, 2); print_mr(src1 + imm, 2, R(rd)));
	INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = SEXT(Mr(src1 + imm, 4), 32); print_mr(src1 + imm, 4, R(rd)));
	INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = src1 < imm);
	INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(rd) = (sword_t)src1 < (sword_t)imm);
	INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << imm); 
	INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (sword_t)src1 >> (imm - 1024));  
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc; s->dnpc = s->pc + imm; 
			IFDEF(CONFIG_FTRACE, if (rd == 1) print_call(s->pc, s->dnpc);));
	...
	INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd) = CSR(imm); CSR(imm) |= src1; IFDEF(CONFIG_ETRACE, print_csr(imm, CSR(imm) | src1)));
	INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw	 , I, R(rd) = CSR(imm); CSR(imm)  = src1; IFDEF(CONFIG_ETRACE, print_csr(imm, src1)));
